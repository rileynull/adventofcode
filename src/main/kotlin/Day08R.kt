    ...
    class BipartiteGraph<T>(val edges: List<Pair<T, T>>) {
        ...
        fun matching(): List<Pair<T, T>> {
            ...
            fun findAlternatingPath(): List<T> {
                ...
                // alternating path.
                val uQueue = ArrayDeque<T>(forwardMapping.keys.filter { it !in uMatching }.shuffled())
                while (uQueue.isNotEmpty()) {
                    val u = uQueue.removeFirst()
                    for (v in forwardMapping[u]!!.shuffled()) {
...
fun main() {
    fun splitToSegments(str: String) =
        str.split(' ').map { it.map { Day08R.Segment.valueOf(it.toUpperCase().toString()) }.toSet()}

    val lines = {}.javaClass.getResource("/day08.txt").openStream().reader().buffered().lineSequence()
            .map { it.split(" | ") }
            .map { halves -> Day08R.Line(splitToSegments(halves[0]), splitToSegments(halves[1])) }
            .toList()
    ...
    val partB = lines.sumBy {
        val possibleMappingsPerWord = it.inputs.map { segments ->
            val possibleMappings = Day08R.SevenSegmentDisplay.digits.values
        ...
        var matching: Map<Day08R.Segment, Day08R.Segment>
        while (true) {
            matching = Day08R.BipartiteGraph(edges).matching().toMap()

            try {
                // check our answer :)
                it.inputs.map { it.map { matching[it]!! }.toSet() }.map(Day08R.SevenSegmentDisplay::decodeSegments)
                it.outputs.map { it.map { matching[it]!! }.toSet() }.map(Day08R.SevenSegmentDisplay::decodeSegments)
                break;
            } catch (_: IllegalStateException) { }
        }

        val unscrambled = it.outputs.map { it.map { matching[it]!! }.toSet() }
        unscrambled.map(Day08R.SevenSegmentDisplay::decodeSegments).joinToString("").toInt()
    }
